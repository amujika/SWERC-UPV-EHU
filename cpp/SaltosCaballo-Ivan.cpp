 ...
 *
 *   Entrada: tres enteros -> N: tamaño de la arista del tablero
 ...
 *   la casilla inicial y final y cada número indica los saltos dados
 *   desde el inicio.
 *
...
bool avanzar_casilla( vector< vector<int> > T, vector< vector<int> > M, const int Size, const int Y, const int X, const int Pasos ) {
	...
	for( k = 0; k < 8; ++k ) {
		if( Y+MovY[k] >= 0 && Y+MovY[k] < Size && X+MovX[k] >= 0 && X+MovX[k] < Size ) {
			...
			i = 0;
			while( i < movimientos && M[Y+MovOrdY[i]][X+MovOrdX[i]] <= M[Y+MovY[k]][X+MovX[k]] ) i++;
			for( int j = movimientos; j > i; --j ) {
				MovOrdY[j] = MovOrdY[j-1];
				MovOrdX[j] = MovOrdX[j-1];
			}
			MovOrdY[i] = MovY[k]; MovOrdX[i] = MovX[k];
			movimientos++;
		}
	}
	...
	k = 0;
	while( k < movimientos && !avanzar_casilla(T,M,Size,Y+MovOrdY[k],X+MovOrdX[k],Pasos+1) ) k++;
